...
function TourCard({ item }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const subsidiaryToursRes = await axios.get(
        `${URL_ANDROID}/subsidiaryTours`
      );
      const relatedSubsidiaryTours =
        subsidiaryToursRes.data?.subsidiaryTours?.filter(
      ...
      if (relatedSubsidiaryTours.length > 0) {
        ...
        const reviewsResponse = await axios.post(
          `${URL_ANDROID}/review-tour/tour-reviews-by-subtours`,
          {
            subTourIds,
          }
        );
      ...
      if (reviews.length > 0) {
        ...
        const averageRating = ((priceQualityAverage + servicesAverage + transportAverage)/3).toFixed(1);
      ...
      if (
        ...
        wishlistResponse.data.favoriteTourIds
      ) {
  ...
  const toggleToWishlist = async () => {
    try {
      ...
      if (fav) {
        await axios.delete(
          `${URL_ANDROID}/favoriteTours/${userId}/${item._id}`
        );
      } else {
      ...
      <View style={Styles.cardContainer}>
        ...
        <View style={Styles.contentContainer}>
          ...
          <View
            style={[
              Styles.skeletonText,
              { width: "80%", height: 15, marginVertical: 8 },
            ]}
          />
          ...
          <View
            style={[
              Styles.skeletonText,
              { width: "40%", height: 13, marginVertical: 7 },
            ]}
          />
          <View
            style={[
              Styles.skeletonText,
              { width: "60%", height: 14, marginTop: 20 },
            ]}
          />
        </View>
      </View>
      ...
      <TouchableOpacity>
        <View style={Styles.cardContainer}>
          <View style={Styles.imageContainer}>
            ...
            <Pressable style={Styles.heartContainer} onPress={toggleToWishlist}>
              {userId ? (
                <AntDesign
                  name="heart"
                  size={23}
