...
const calculateAverageRating = (reviews) => {
  ...
  const averageRating = (
    (priceQualityAverage + servicesAverage + transportAverage) /
    3
  ).toFixed(1);
...
function TourCart({
  ...
  const getReview = async (subTourIds) => {      
    try {
  ...
  useEffect(() => {
    const getSubTourIds = async () => {
      try {
        const subTourIds = [];
        const response = await axios.get(
          `${URL_ANDROID}/subsidiaryTours/getSubToursByOrgTourId/${item.originalTourId._id}`
        );
        const subTours = response.data.subsidiaryTours;
        subTours.map((subTour) => {
          subTourIds.push(subTour._id);
        });
        getReview(subTourIds);
      } catch (error) {
        console.log("Error getting subTourIds: ", error);
      }
    };
    getSubTourIds();
  }, []);
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={styles.tourCardFooter}>
              ...
              <View style={styles.tourCardFooterAvgScore}>
                ...
                <Text style={styles.tourCardFooterAvgScoreText}>
