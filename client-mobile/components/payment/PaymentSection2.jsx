...
export default function PaymentSection2({
  bookingInfo,
  order,
  bookingId,
  paymentStatus,
  ...
  React.useEffect(() => {
    const fetchDiscount = async () => {
      try {
        const response = await axios.get(`${URL_ANDROID}/discount/customer/${bookingInfo.userId}`);
        console.log("Discounts:", response.data);
        
        if (response.data.success) {
          const { activeDiscounts } = response.data;
          const sortedActive = activeDiscounts.sort(
  ...
  React.useEffect(() => {
    const updateBooking = async () => {
      if (!bookingId || finalPrice === undefined || !discountCodeSelected)
        return;

      try {
        const response = await axios.put(
          `${URL_ANDROID}/booking/update/${bookingId}`,
          { finalPrice, discountId: discountCodeSelected.discountId }
        );
      } catch (error) {
        console.error(
          ...
          >
              ...
              <View style={styles.itemHeader}>
                ...
                >
                  <Image
                    ...
                    borderRadius={4}
                    source={{
                      uri: `http://192.168.32.1:5000${bookingInfo.thumbnail}`,
                    }}
                  />
                </View>
            ...
            {discountCodeSelected ? (
              ...
              >
                ...
                >
                  Discount Code Aplied:
                  <Text style={{ color: Colors.blackColorText }}>
                    &nbsp;{discountCodeSelected.discountCode}
                  </Text>
                </Text>
              </Pressable>
            ) : (
                ...
                {discountCodeSelected && (
                  <>
                    <Text style={{ textAlign: "right", color: Colors.red }}>
                      - ₫{discountCodeSelected.discountPrice}
                    </Text>
                    <Text
                      style={{
                        textAlign: "right",
            ...
            >
              ...
              <View style={{ paddingVertical: 4 }}>
                ...
                <Text>Until 11:59 PM on June 19, 2025</Text>
              </View>
            </View>
            {/* Text policy */}
            <View style={{ marginVertical: 6, flexDirection: "column", gap: 6, marginTop: 15 }}>
              <Text
                style={{       
                  fontSize: 14,
                  color: Colors.blackColorText,
              ...
              >
                By continuing, you agree to all TravelNow policies and the
                activity provider's terms and conditions.
              </Text>
              <Text style={{
                  fontSize: 14,
                  color: Colors.blackColorText,
                ...
                You can request a refund within 24 hours of booking, provided
