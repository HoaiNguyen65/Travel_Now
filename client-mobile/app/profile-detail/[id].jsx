...
const ProfileDetailScreen = () => {
  ...
  const [isEditAvatar, setIsAvatar] = useState(false);
  ...
  const handlePickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      formData.append("thumbnail", {
        uri: file.uri,
        name: file.uri.split("/").pop() || `avatar_${Date.now()}.jpg`,
      ...
      try {
          ...
          handleChange("avatar", response.data.thumbnailPath);
      } catch (error) {
  ...
  const uri = `${URL_ANDROID}${userData?.avatar}`
  console.log("uri: ",uri);
    ...
    >
      ...
      <View style={styles.avatarContainer}>
        <TouchableOpacity onPress={handlePickImage}>
          <Image
            source={{
              uri: `${URL_ANDROID}${userData.avatar}`,
            }}
            style={styles.avatar}
          />  
        </TouchableOpacity>
