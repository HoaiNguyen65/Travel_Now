...
const TicketScreen = () => {
  ...
  const [isTicketInfoModalVisible, setIsTicketInfoModalVisible] = useState(false);
  ...
  const fetchTickets = useCallback(async (userId) => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(
        `${URL_ANDROID}/ticket/customer/get-all-tickets?userId=${userId}`
      );
  ...
  useEffect(() => {
    ...
    if (userId) {
      ...
      wsRef.current = connectWebSocket("/ws/cart", 
        // Message handler
        (data) => { 
          if (data.type === "paymentComplete") {
            // Refresh tickets after payment
            ...
            if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
              ...
              console.log(`Sent removeItem for cartItemId: ${cartItemId}`);
            }
          }
        },
        // Close handler  
        () => {
          console.log("Cart WebSocket closed");
        }
      );
  ...
  const handleBookAgain = async (item) => {
    try {
      ...
      if (status === "success") {
        console.log("zo success book again");
        ...
        const [bookingRes, reviewsRes] = await Promise.all([
          ...
          axios.get(
            `${URL_ANDROID}/review-tour/tour-reviews/${item.bookingId.subTourId._id}`
          ),
        ]);
      ...
      fetchTickets(userId);
      Alert.alert("Notification", error.response.data.message || "Something went wrong. Please try again later.");
    }
  };
  ...
  const renderTicketItem = ({ item }) => {
    ...
    if (activeTab === "Completed" && reviews.length > 0) {
      const review = reviews.find(
        (review) => item.bookingId.subTourId._id === review.subTourId._id
      );  
