...
function Discover() {
  ...
  const handlePickImage = async () => {
    console.log("handlePickImage at homepage activate");
    ...
    if (!result.canceled && result.assets.length > 0) {
      ...
      formData.append("file", {
        ...
        type: mime.getType(selectedAsset.uri) || "image/jpeg",
      ...
      try {
        ...
        const response = await axios.post("http://10.0.2.2:9999", formData, {
          headers: {
        ...
        const placeFromAI = response.data.extra; // e.g., "cantho"
  
        // Filter tours based on the place returned from AI
        const filteredTours = tours.filter((tour) =>
          Array.isArray(tour.place) && tour.place.includes(placeFromAI)
        );
  
        // Update state with filtered tours
        setSearchResults(filteredTours);
        setTempTour(filteredTours);
        setTotalTour(filteredTours.length);
        setIsNotFound(filteredTours.length === 0);
        setQuery(placeFromAI); // Optionally set the query to show the place in the search bar
