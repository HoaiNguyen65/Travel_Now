...
class CustomersController {
  ...
  async verifyRegistrationOTP(req, res) {
    try {
      ...
      // Now that OTP is verified, create the user account with stored data
      const userData = storedData.userData;

      // Create the account using the stored registration data
      Account.register(
        new Account({
  ...
  async resendOTP(req, res) {
    try {
      ...
      const otp = generateOTP();
      // Keep the original registration data but update OTP and timestamp
      otpStore.set(email, {
        ...storedData,
        otp,
        timestamp: Date.now(),
  ...
  async verifyEmailOTP(req, res) {
    try {
      ...
      // Mark OTP as verified but keep it in store for registration completion
      otpStore.set(email, {
        ...storedData,
        verified: true,
      });
