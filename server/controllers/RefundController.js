...
class RefundController {
  ...
  async createRefundRequest(req, res, next) {
    try {
      ...
      const { accountNumber, bankName, accountNameBank, reason } =
        refundInformation;
      if (!accountNumber || !bankName || !accountNameBank || !reason) {
        return res.status(400).json({
          status: "error",
          message:
            "accountNumber, bankName, reason and accountNameBank are required in refundInformation",
        });
      }
      ...
      // Tạo thông báo cho admin về việc customer yêu cầu refund
      const account = await Account.findById(userId);
      const ticket = await Ticket.findById(ticketId).populate("bookingId");
      const orgTour = await OriginalTour.findById(
        ticket.bookingId.originalTourId
      );
      // Create notification
      const notification = await Notification.create({
        type: "REFUND",
        message: `${account.accountCode} requested a refund for ticket ${ticket.ticketRef}`,
        information: {
          refund: {
            ticketCode: ticket.ticketRef,
            tourName: orgTour.title,
            username: account.username,
            amount: ticket.bookingId.finalPrice,
            userId: userId,
          },
        },
  ...
  async updateRefundRequest(req, res, next) {
    try {
      ...
      const ticket = await Ticket.findById(refund.ticketId).populate(
        "bookingId"
      );
      const orgTour = await OriginalTour.findById(
        ticket.bookingId.originalTourId
      );
      const refundBy = await Account.findById(refund.refundBy);
      const customer = await Account.findById(refund.userId);
      // Create notification
      const notification = await Notification.create({
        type: "REFUND",
        message: `${refundBy.accountCode} has refunded the ticket ${ticket.ticketRef} for customer ${customer.accountCode}`,
        information: {
          refund: {
            ticketCode: ticket.ticketRef,
            tourName: orgTour.title,
