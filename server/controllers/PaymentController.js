...
const { getPaymentSocket, getCartSocket } = require("../websocket");
    ...
    try {
      ...
      if (code === "00") {
        ...
        try {
          ...
          // remove item in cart if booking from cart
          const cart = await Cart.findOne({ userId: booking.userId });
          if (cart) {
            // Find and remove the cart item for this tour
            const cartItemIndex = cart.cartIds.findIndex(
              (item) =>
                item.originalTourId.toString() ===
                  booking.originalTourId.toString() &&
                item.subTourId.toString() === booking.subTourId.toString()
            );
  
            if (cartItemIndex > -1) {
              const cartItemId = cart.cartIds[cartItemIndex]._id;
              cart.cartIds.splice(cartItemIndex, 1);
              await cart.save();
              console.log(`✅ Removed tour from cart for user ${booking.userId}`);
  
              // Send WebSocket message with cartItemId
              const wss = getCartSocket();
        ...
        const subTour = await SubsidiaryTour.findById(booking.subTourId);
        const discount = await Discount.findById(booking.discountId);
        const account = await Account.findById(booking.userId);
        // Create notification
        const notification = await Notification.create({
          type: "BOOK TOUR",
          message: `${account.accountCode} book tour ${subTour.subTourCode} successfully`,
          information: {
            bookTour: {
              mail: booking.buyerEmail,
              tourCode: subTour.subTourCode,
