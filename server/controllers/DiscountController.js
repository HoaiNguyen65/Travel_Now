...
class DiscountController {
  // [GET] /discount/customer/:cusId
  async getDiscountByCustomerId(req, res, next) {
    try {
      const userId = req.params.cusId; // Giả sử customerId được truyền qua params

      // Tìm tất cả các discount đã được người dùng sử dụng
      const usedDiscounts = await UsedDiscount.find({ userId }).select(
        "discountId"
      );

      // Lấy danh sách discountId đã sử dụng
      const usedDiscountIds = usedDiscounts.map((used) => used.discountId);
      console.log("Used Discount IDs:", usedDiscountIds);

      await Discount.updateMany(
        {
          discountDateEnd: { $lt: new Date() },
          isActive: true,
        },
        {
          $set: { isActive: false },
      ...
      // Tìm các discount active
      const activeDiscounts = await Discount.find({
        _id: { $nin: usedDiscountIds }, // Loại bỏ các discount đã sử dụng
        isActive: true,
        discountAvailableSlots: { $gt: 0 }, // Còn slot khả dụng
        discountDateStart: { $lte: new Date() }, // Bắt đầu trước hoặc bằng hiện tại
        discountDateEnd: { $gte: new Date() }, // Kết thúc sau hoặc bằng hiện tại
      });

      // Tìm các discount inactive
      const inactiveDiscounts = await Discount.find({
        isActive: false,
      });

      // Trả về danh sách discount active và inactive
